...
<div class="table-header">
	<div class="search-wrap">
		<form action="<c:url value='/admin/reservations' />" method="get"
			id="searchForm">
			<select name="searchType" id="searchType"
				...
				<option value="movieTitle"
					${currentSearchType eq 'movieTitle' ? 'selected' : ''}>제목</option>
				<option value="userId"
					${currentSearchType eq 'userId' ? 'selected' : ''}>예매자</option>
				<option value="screeningTime"
					${currentSearchType eq 'screeningTime' ? 'selected' : ''}>상영시간</option>
				<option value="reservationDate"
					${currentSearchType eq 'reservationDate' ? 'selected' : ''}>결제일시</option>
			</select> <input type="text" name="keyword" id="keyword"
				class="form-control search-input" placeholder="제목을 입력해 주세요"
				value="${currentKeyword}">
			<button type="submit" class="btn btn-primary search-btn">검색</button>

			<select name="sortCriteria" id="sortCriteria"
				class="form-select sort-select" onchange="submitSort()">
				<option value="default">-- 정렬 기준 --</option>
				<option value="screeningTime"
					${currentSortCriteria eq 'screeningTime' ? 'selected' : ''}>상영시간순</option>
				<option value="reservationDate"
					${currentSortCriteria eq 'reservationDate' ? 'selected' : ''}>결제일시순</option>
				<option value="status"
					${currentSortCriteria eq 'status' ? 'selected' : ''}>상태순</option>
			</select>
...
        					</c:otherwise>
					...
					<td>
						<button class="action-btn view-content-btn"
							onclick="openReservationDetail(${reservations.id})">상세보기</button>
					</td>
				</tr>
			</c:forEach>
		</tbody>
...
<div id="reservationDetailModal" class="modal">
	<div class="modal-content">
		...
		<div class="modal-actions">
			<button class="action-btn approve-btn" onclick="refundFromModal()">환불하기</button>
			</div>
	</div>
</div>
<script src="<c:url value='/resources/js/admin_reservations.js' />"></script>
