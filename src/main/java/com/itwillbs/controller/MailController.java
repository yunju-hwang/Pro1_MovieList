...
public class MailController {
    ...
    public String sendIdByEmail(@RequestParam("email") String userEmail, Model model) {
        
        // MailService에 DB 조회 및 이메일 발송 작업을 모두 위임
        boolean success = mailService.findIdAndSendEmail(userEmail); 
        
        if (success) {
            model.addAttribute("msg", "아이디가 이메일로 전송되었습니다. 확인해 주세요.");
        } else {
            // 이메일이 DB에 없거나 발송에 실패한 경우
            model.addAttribute("msg", "해당 이메일로 등록된 아이디가 없거나, 발송 중 오류가 발생했습니다.");
        }
    ...
    public String sendPwByEmail(@RequestParam("userId") String userId, 
        ...
        if (success) {
            model.addAttribute("msg", "임시 비밀번호가 이메일로 전송되었습니다. 확인 후 로그인해 주세요.");
        } else {
            model.addAttribute("msg", "아이디 또는 이메일 정보가 일치하지 않거나, 발송 중 오류가 발생했습니다.");
        }
